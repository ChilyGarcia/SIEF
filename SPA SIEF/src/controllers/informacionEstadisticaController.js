...
export default () => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    var subMenuItems = document.querySelectorAll(
      ".main-menu .has-subnav .submenu li a"
    );

    subMenuItems.forEach(function (item) {
      item.addEventListener("click", function (event) {
        event.preventDefault();
        var menuItemId = item.getAttribute("data-menu-id");
        var codigoPrograma = menuItemId;

        const url =
          "http://localhost:8080/programas/listaProgramaPorCodigo/" +
          codigoPrograma;
